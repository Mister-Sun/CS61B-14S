...
public class PixImage {
    ...
    public PixImage boxBlur(int numIterations) {
        // Replace the following line with your solution.
        PixImage copy = new PixImage(gridWidth, gridHeight);
        if (numIterations <= 0) {
            return this;
        } else {
            for (int w = 0; w < gridWidth; w++) {
                for (int h = 0; h < gridHeight; h++) {
                    int[] sum = sumRGB(w, h, this);
                    if ((w == 0 || w == gridWidth - 1) && (h == 0 || h == gridHeight - 1)) {
                        copy.setPixel(w, h, (short) (sum[0] / 4), (short) (sum[1] / 4), (short) (sum[2] / 4));
                    } else if (((w == 0 || w == gridWidth - 1) && (h > 0 && h < gridHeight))
                            || ((w > 0 && w < gridWidth - 1) && (h == 0 || h == gridHeight - 1))) {
                        copy.setPixel(w, h, (short) (sum[0] / 6), (short) (sum[1] / 6), (short) (sum[2] / 6));
                    } else {
                        copy.setPixel(w, h, (short) (sum[0] / 9), (short) (sum[1] / 9), (short) (sum[2] / 9));
                    }
                }
            }
        }
    ...
    private int[] sumRGB(int w, int h, PixImage p) {
        int []sum = new int[3];
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (isInside(w + i, h + j)) {
                    sum[0] += p.getRed(w + i, h + j);
                    sum[1] += p.getGreen(w + i, h + j);
                    sum[2] += p.getBlue(w + i, h + j);
                }
            }
        }
        return sum;
    ...
    private boolean isInside(int w, int h) {
        return w >= 0 && w < gridWidth && h >= 0 && h < gridHeight;
    }
    ...
    public PixImage sobelEdges() {
        // Replace the following line with your solution.
        PixImage curr = new PixImage(gridWidth, gridHeight);
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (isInside(x + i, y + j)) {
                    ...
                    a = edges(x + i, gridWidth);
                    b = edges(y + j, gridHeight);
                }
