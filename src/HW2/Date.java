...
class Date {
    ...
    public Date(String s) {
        String[] set = s.split("/");
        int month = Integer.parseInt(set[0]);
        int day = Integer.parseInt(set[1]);
        int year = Integer.parseInt(set[2]);
        if (isValidDate(month, day, year)) {
            this.day = day;
            this.year = year;
            this.month = month;
    ...
    public int difference(Date d) {
        ...
        if (!isAfter(d) && !isBefore(d)) {
            ...
            return countDays(d, this);
        } else {
            return -countDays(this, d);
        }
    }
    ...
    public int countDays(Date d1, Date d2) {
        ...
        if (yearDifference > 1) {
            for (int y = d1.year; y < d2.year - 1; y++) {
                if (isLeapYear(y)) {
                    difference += 366;
                } else {
                    difference += 365;
            ...
            for (int m = d1.month; m < 13; m++) {
                difference += daysInMonth(m, d2.year - 1);
            }
            for (int m = 1; m < d2.month; m++) {
                difference += daysInMonth(m, d2.year);
            }
            difference += d2.day - d1.day;
            return difference;
